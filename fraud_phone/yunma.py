...
#-*- coding: utf-8 -*-
'''
Created on 2017-04-07 14:40:43
...
# 账号/密码: kkk111(08c4ab4b3be7bea180ccc6e86412208b8146ac9e)
# 登录后, token有效期为10分钟, 过期需要重新申请token
# 平台接口前缀: 'http://api.yzm7.com'
'''
账户基本信息
account_info = {
    'user_info': {
        ...
        u'真实姓名': '李龙',
        u'支付宝': '',
        u'电子邮箱': '460454865@qq.com',
        u'手机号码': '18926779674',
        u'QQ': '1747485592',
        u'注册日期': '2017/3/7 14:26:44',
    }

    u'Developer': 'FMsSY0z6CJDhIm04raAY6w%3d%3d',
}
'''
...
class Yunma(object):
    def __init__(self):
        self.api_prefix = r'http://api.yzm7.com'
        self.developer = r'FMsSY0z6CJDhIm04raAY6w%3d%3d'
        self.user_name = 'kkk111'
        ...
        # 保存网站项目信息的文件
        self.items_file = os.path.sep.join([sys.path[0], 'items', 'yunma_items.json'])

        # 保存最终获得的手机号码信息保存文件
        current_date = time.strftime('%Y-%m-%d', time.localtime(time.time()))
        self.phone_file = sys.path[0] + os.path.sep + 'data' + os.path.sep + 'ema_phone_yunma_phone_%s.txt' % (current_date)
        # self.phone_file = sys.path[0] + os.path.sep + 'data' + os.path.sep + 'yunma_phone_%s.txt' % (current_date)

        # LOG
        month = time.strftime('%Y-%m', time.localtime(time.time()))
        self.log_file = os.path.sep.join([sys.path[0], 'log', 'yunma_%s.log' % month])
    ...
    # 获取区域
    def get_area(self):
        area_url = 'http://api.yzm7.com:7777/GetArea?'
...
def main():
    ...
    if os.path.exists(yunma.items_file):
        items = get_items_from_file(yunma.items_file)
        yunma.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(yunma.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = yunma.get_items()
        save_items_with_json(items, yunma.items_file)
        yunma.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(yunma.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            yunma.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))
            ...
            if item_id and item_price <= 10:
                ...
                nums = yunma.get_phone(item_id, count)
                yunma.logger.info(nums)
                if len(nums) == 0:
                    yunma.logger.info(u'没有获取到有效号码')
                ...
                if len(phone_items) > 0:
                    save_items(phone_items, yunma.phone_file)
                    yunma.release_phone()
                ...
                yunma.logger.info(u'项目价格超过10元, 放弃.')
