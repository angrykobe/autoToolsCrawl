...
# -*- coding: utf-8 -*-
'''
Created on 2017-04-06 17:32:44
...
# 老头(http://www.ltxin.com/userManage/index.aspx)
...
account_info = {
    ...
    'Developer': 'PPTzVZvlGXwIduRmYzyDQQ%3d%3d'
}
'''
...
class Laotou(object):
    def __init__(self):
        self.api_prefix = r'http://xapi.ltxin.com'
        self.developer = r'PPTzVZvlGXwIduRmYzyDQQ%3d%3d'
        ...
        # 保存网站项目信息的文件
        self.items_file = os.path.sep.join([sys.path[0] , 'items', 'laotou_items.json'])

        # 保存最终获得的手机号码信息保存文件
        current_date = time.strftime('%Y-%m-%d', time.localtime(time.time()))
        self.phone_file = sys.path[0] + os.path.sep + 'data' + os.path.sep + 'laotou_phone_%s.txt' % (current_date)

        # LOG
        month = time.strftime('%Y-%m', time.localtime(time.time()))
        self.log_file = os.path.sep.join([sys.path[0], 'log', 'laotou_%s.log' % month])
    ...
    # 获取区域
    def get_area(self):
        area_url = 'http://api.ltxin.com:8888/GetArea?code=utf8'
...
def main():
    ...
    if os.path.exists(laotou.items_file):
        items = get_items_from_file(laotou.items_file)
        laotou.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(laotou.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = laotou.get_items()
        save_items(items, laotou.items_file)
        laotou.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(laotou.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            laotou.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))
            ...
            if item_id and item_price <= 10:
                ...
                nums = laotou.get_phone(item_id, count)
                laotou.logger.info(nums)
                if len(nums) == 0:
                    laotou.logger.info(u'没有获取到有效号码')
                ...
                if len(phone_items) > 0:
                    save_items(phone_items, laotou.phone_file)
                ...
                laotou.logger.info(u'项目价格超过10元, 放弃.')
