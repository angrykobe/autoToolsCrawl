...
# 百万码(www.baiwanma.com)
# 账号/密码: hbbhbb(hbb123)
# 如果在10分钟没有再次用token访问其他接口信息，则登录token会被系统自动回收
# 目前该网站的退出API无效，会导致最大在线数问题
# 平台接口前缀: 'http://api.baiwanma.com'
'''
账户基本信息
account_info = {
    'user_info': {
...
class BaiWanMa(object):
    ...
    def get_phone(self, item_id, count=1, phone=None):
        ...
        payload = {
            ...
            'Area': u'不限',        # 区域(不填则随机)
            'PhoneType': '0',        # 运营商(不填为0, [0-随机][1-移动][2-联通][3-电信])
            "Code": "utf8"
        }
