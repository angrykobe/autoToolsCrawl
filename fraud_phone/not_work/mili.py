...
# 淘码(http://www.mili02.com:9000/)
# 账号/密码: hbbhbb(rfM#!EzZU%!3s7*kxbTy)
# 平台接口前缀: 'http://www.mili02.com:9180/service.asmx'
'''
账户基本信息
account_info = {
    'user_info': {
...
class MiLiMa(object):
    def __init__(self):
        ...
        self.api_prefix = r'http://www.mili02.com:9180/service.asmx'
...
def main():
    milima = MiLiMa()
    milima.login()
    milima.release_all_phone()

    if os.path.exists(milima.items_file):
        items = get_items_from_file(milima.items_file)
        milima.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(milima.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = milima.get_items()
        save_items_with_json(items, milima.items_file)
        milima.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(milima.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            milima.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))
            ...
            if item_price <= 10:
                if item_price > 1:
                    ...
                    count = 10
                nums = milima.get_phone(item_id, count)
                milima.logger.info(nums)
                if len(nums) == 0:
                    milima.logger.info(u'没有获取到有效号码！')
                ...
                if len(phone_items) > 0:
                    save_items(phone_items, milima.phone_file)
                ...
                milima.logger.info(u'项目价格超过10元, 放弃！')
