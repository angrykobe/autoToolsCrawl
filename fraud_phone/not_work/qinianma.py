...
# 七年码(www.qinian365.com/)
# 账号/密码: hbbhbb(rfM#!EzZU%!3s7*kxbTy)
# 平台接口前缀: 'http://47.52.114.154:9180/service.asmx'
'''
账户基本信息
account_info = {
    'user_info': {
...
class QiNianMa(object):
    def __init__(self):
        # 该参数用来存放该平台获取 项目列表 的地址
        self.api_prefix = r'http://47.52.114.154:9180/service.asmx'
...
def main():
    qinianma = QiNianMa()
    qinianma.login()
    qinianma.release_all_phone()

    if os.path.exists(qinianma.items_file):
        items = get_items_from_file(qinianma.items_file)
        qinianma.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(qinianma.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = qinianma.get_items()
        save_items_with_json(items, qinianma.items_file)
        qinianma.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(qinianma.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            qinianma.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))
            ...
            if item_price <= 1:
                if item_price > 1:
                    ...
                    count = 10
                nums = qinianma.get_phone(item_id, count)
                qinianma.logger.info(nums)
                if len(nums) == 0:
                    qinianma.logger.info(u'没有获取到有效号码！')
                ...
                for num in nums:
                    ...
                    # 加黑已取号码
                    qinianma.add_black_list(item_id, num)

                    phone_items.append(new_item)
                if len(phone_items) > 0:
                    save_items(phone_items, qinianma.phone_file)
                ...
                qinianma.logger.info(u'项目价格超过10元, 放弃！')
    ...
    qinianma.logger.info(u'采集结束' + '-' * 30)
