...
Updated on 2017-05-11 11:32:53
...
# E码(http://www.ema666.com/userManage/index.aspx)
# 账号/密码: aaa111111(08c4ab4b3be7bea180ccc6e86412208b8146ac9e)
# 登录后, token有效期为10分钟, 过期需要重新申请token
# 平台接口前缀: 'http://api.ema666.com'
'''
账户基本信息
account_info = {
    'user_info': {
        ...
        u'手机号码': '13960219024',
        u'QQ': '460454865',
        u'注册日期': '2017/3/7 11:41:42',
    }

    # 开发者参数

    'Developer': '5REzuw%2bYc3ZjgNpa4mHWOw%3d%3d'
}
'''
...
class Ema(object):
    def __init__(self):
        self.api_prefix = r'http://api.ema666.com'
        self.developer = r'5REzuw%2bYc3ZjgNpa4mHWOw%3d%3d'
        ...
        self.headers = {
            ...
            'Referer': 'http://www.ema666.com/centent.html',
            'Accept-Encoding': 'gzip, deflate, sdch',
            'Accept-Language': 'zh-CN,zh;q=0.8,en-US;q=0.6,en;q=0.4',
        }
        # 保存网站项目信息的文件
        self.items_file = os.path.sep.join([sys.path[0] , 'items', 'ema_items.json'])

        # 保存最终获得的手机号码信息保存文件
        current_date = time.strftime('%Y-%m-%d', time.localtime(time.time()))
        self.phone_file = sys.path[0] + os.path.sep + 'data' + os.path.sep + 'ema_phone_%s.txt' % (current_date)

        # LOG
        month = time.strftime('%Y-%m', time.localtime(time.time()))
        self.log_file = os.path.sep.join([sys.path[0], 'log', 'ema_%s.log' % month])
    ...
    def login(self):
        ...
        url = self.api_prefix + '/Api/userLogin'
    ...
    def get_items(self):
        ...
        url = self.api_prefix+'/Api/userGetItems'
    ...
    # 获取区域
    def get_area(self):
        area_url = 'http://api.ema666.com/Api/userGetArea?code=utf8'
    ...
    def get_phone(self, item_id, count=1, phone=''):
        ...
        url = self.api_prefix + '/Api/userGetPhone'
    ...
    def release_all_phone(self):
        url = self.api_prefix + '/Api/userReleaseAllPhone'
    ...
    # 退出
    def exit(self):
        url = self.api_prefix + '/Api/userExit'
        payload = {'token': self.token}
        text = self.request('get', url, payload)
        if text:
...
def main():
    ...
    if os.path.exists(ema.items_file):
        items = get_items_from_file(ema.items_file)
        ema.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(ema.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = ema.get_items()
        save_items(items, ema.items_file)
        ema.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(ema.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            ema.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))
            ...
            if item_id and item_price <= 10:
                ...
                nums = ema.get_phone(item_id, count)
                ema.logger.info(nums)
                if len(nums) == 0:
                    ema.logger.info(u'没有获取到有效号码')
                ...
                if len(phone_items) > 0:
                    save_items(phone_items, ema.phone_file)
                ...
                ema.logger.info(u'项目价格超过10元, 放弃.')
