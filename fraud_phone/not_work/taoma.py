...
# 平台接口前缀: 'http://47.90.98.47:9180/service.asmx/'
'''
账户基本信息
account_info = {
    'user_info': {
...
class TaoMa(object):
    def __init__(self):
        ...
        self.api_prefix = r'http://47.90.98.47:9180/service.asmx/'
...
def main():
    taoma = TaoMa()
    taoma.login()
    taoma.release_all_phone()

    if os.path.exists(taoma.items_file):
        items = get_items_from_file(taoma.items_file)
        taoma.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(taoma.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = taoma.get_items()
        save_items_with_json(items, taoma.items_file)
        taoma.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(taoma.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            taoma.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))
            ...
            if item_price <= 10:
                ...
                nums = taoma.get_phone(item_id, count)
                taoma.logger.info(nums)
                if len(nums) == 0:
                    taoma.logger.info(u'没有获取到有效号码！')
                ...
                for num in nums:
                    ...
                    taoma.add_black_list(item_id, num)

                    phone_items.append(new_item)
                if len(phone_items) > 0:
                    save_items(phone_items, taoma.phone_file)
                ...
                taoma.logger.info(u'项目价格超过10元, 放弃！')
