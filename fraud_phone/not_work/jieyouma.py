...
# 解忧码(http://jieyouma.com/index.html)
# 账号/密码: hbbhbb(rfM#!EzZU%!3s7*kxbTy)
# 平台接口前缀: 'http://120.78.76.197:9180/service.asmx/'
'''
账户基本信息
account_info = {
    'user_info': {
...
class JieYouMa(object):
    def __init__(self):
        # 该参数用来存放该平台获取 项目列表 的地址
        self.api_prefix = r'http://120.78.76.197:9180/service.asmx/'
...
def main():
    jieyouma = JieYouMa()
    jieyouma.login()
    jieyouma.release_all_phone()

    if os.path.exists(jieyouma.items_file):
        items = get_items_from_file(jieyouma.items_file)
        jieyouma.logger.info(u'从文件中读取__%s__网站可接收验证码项目%d个。' % (
            str(jieyouma.__class__).split('.')[1].strip("'>"), len(items)))
    else:
        items = jieyouma.get_items()
        save_items_with_json(items, jieyouma.items_file)
        jieyouma.logger.info(u'一共获取__%s__网站可接收验证码项目%d个。' % (
            str(jieyouma.__class__).split('.')[1].strip("'>"), len(items)))
    ...
    for item in items[:]:
        ...
        while 1:
            ...
            jieyouma.logger.info(u'项目: __%s__, ID: __%s__, 价格: __%s__' % (item_name, item_id, item_price))

            if item_price <= 1:
                count = 20

                nums = jieyouma.get_phone(item_id, count)

                jieyouma.logger.info(nums)
                if len(nums) == 0:
                    jieyouma.logger.info(u'没有获取到有效号码！')
                ...
                for num in nums:
                    ...
                    # 加黑已取号码
                    jieyouma.add_black_list(item_id, num)

                    phone_items.append(new_item)
                if len(phone_items) > 0:
                    save_items(phone_items, jieyouma.phone_file)
                ...
                jieyouma.logger.info(u'项目价格超过10元, 放弃！')
    ...
    jieyouma.logger.info(u'采集结束' + '-' * 30)
