...
def get_docIds_inRange(mydb,start,last):
    sql = '''SELECT * FROM docIds where rowid>%d and rowid<%d'''%(start,last)
    data = mydb.fetchall(sql)
    return data

def get_docIds_All(mydb):
    sql = '''SELECT rowid,case_name,judi_date,summary,case_no,docid,judi_process,court_name,private_reason,case_type,beDownloaded FROM docIds where beDownloaded=0'''
    data = mydb.fetchall(sql)
    return data
